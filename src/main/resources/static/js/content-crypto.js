...
// share viewer) so the shareScope->KEK branching lives in exactly one place (CLAUDE.md's
...
window.OE_CONTENT_CRYPTO = (function() {
  ...
  // JSON {iv, ciphertext} record (as text) when encrypted. shareScope: 'private' unwraps with
  // the caller's personal key; anything else ('workspace'/'collabo') unwraps with the cached
  // workspace key. Throws if the required session key isn't cached yet.
  async function decrypt(wrappedContentKeyStr, hostsContentStr, shareScope) {
