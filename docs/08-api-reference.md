...
| Method | Path | 설명 |
|---|---|---|
| GET | `/api/admin/users` | 멤버 검색/목록 (`?q=`) |
...
| POST | `/api/admin/workspace/rotate` | 워크스페이스키 회전 실행 (감사 로그 대상) |
| GET/POST | `/api/admin/invites` | 초대 코드 목록/발급 (`workspace` 전용) |
| GET/POST/PATCH/DELETE | `/api/admin/teams` (`+/{teamNo}`) | 팀 CRUD (`workspace` 전용) |
| GET/POST/PATCH/DELETE | `/api/admin/hosts/ua`, `/api/admin/hosts/url` (`+/{id}`, `+/order`) | 관리자 전역 UA/URL 프리셋 CRUD |

## 관리자 — 감사 로그 (`/api/admin/audit-log`)

| Method | Path | 설명 |
|---|---|---|
| GET | `/api/admin/audit-log?page=` | 내 워크스페이스의 Tier-1 감사 로그 (50건씩 페이지네이션) |

기록되는 액션 종류와 스코프 규칙은 [07-database-schema.md](07-database-schema.md)의
`HUB_AUDIT_LOG`를 참고.

## 관리자 — 워크스페이스 콘솔 (`/api/admin/workspaces/*`, `workspace` 모드의 인스턴스 `adm` 전용)
...
| Method | Path | 설명 |
|---|---|---|
| POST | `/api/admin/settings/oid-domain-default` | oeOID 기본 도메인 목록 저장 |
| POST | `/api/admin/settings/backup-interval` | H2 자동 백업 주기 저장 |
| POST | `/api/admin/settings/identifier` | IP 식별 폴백 on/off |
| POST | `/api/admin/settings/trust-internal-cert` | 내부망 백엔드 인증서 신뢰 on/off |
| POST | `/api/admin/settings/fwdproxy-whitelist` | 포워드 프록시 릴레이 화이트리스트 저장 |

`/oehub/settings/ca/generate`, `/oehub/settings/ca/import`(폼 POST, JSON 아님)도 이 그룹에
