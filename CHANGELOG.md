...
- Tier-1(보안/접근권한) 감사 로그 추가 — 가입 승인/거부, 역할 변경, 계정 삭제, 관리자의 비밀번호 강제 재설정, 루트 CA 재발급/가져오기, 워크스페이스 상태 변경, 워크스페이스키 회전을 무기한 기록합니다. 워크스페이스 관리자(또는 self-hosted의 admin)가 자기 워크스페이스분만 조회할 수 있는 전용 화면(설정 > 감사 로그)을 제공합니다.
- 오픈소스 공개를 위한 설계 문서 세트(`docs/`) 추가 — 개요, oeHosts/oelink, oeProxy, 배포 모드, 종단간 암호화, 보안 위협 모델, 데이터베이스 스키마, API 레퍼런스를 다룹니다.

### 변경됨
- `workspace` 모드에서 관리자의 "비밀번호 초기화"(멤버 비밀번호 강제 재설정) 기능을 제거했습니다 — 이 액션은 대상자의 `wrapped_private_key`를 새 비밀번호로 재래핑하지 못해(원래 비밀번호가 필요하지만 관리자는 알 수 없음), 초기화된 멤버가 자신의 워크스페이스 콘텐츠를 더 이상 복호화하지 못하게 되는 문제가 있었습니다. `self-hosted` 모드는 신원 암호화가 더미 placeholder라 영향이 없으므로 그대로 유지됩니다. workspace 모드에서 잠긴 계정을 되찾는 유일한 방법은 본인의 복구 코드를 통한 `/recover` 셀프서비스 플로우입니다.
...
- `self-hosted` 모드는 더 이상 쓰이지 않는 신원 암호화 키 자료(RSA 키쌍, 복구 코드 등)를 생성하지 않습니다 — 이 모드에서는 아무것도 그 키로 암호화되지 않으므로 순수한 오버헤드였습니다.
...
- 실행 옵션이 전부 `-Doe.<이름>` 형식으로 통일되었습니다(`-Dport`→`-Doe.port`, `-Dhome`→`-Doe.home`, `-Ddev`→`-Doe.dev`, `-Dapp.version`→`-Doe.app.version`; `-Doe.mode`는 이름 자체는 기존과 동일). 새 `-Doe.data.dir` 옵션으로 H2 데이터 디렉터리를 홈 디렉터리와 별도 위치로 지정할 수 있습니다.
- `-Doe.mode`의 기본값(단일 워크스페이스 배포)을 가리키는 값을 `standalone`에서 `self-hosted`로 변경했습니다. "혼자 있는"이라는 뜻보다 "직접 호스팅한다"는 의미를 더 정확히 드러내기 위함입니다.
- "로그인 상태 유지" 로그인의 JWT/인증 쿠키 만료 기간을 365일에서 30일로 단축했습니다 — 탈취된 쿠키가 자격 증명으로 쓰일 수 있는 기간을 줄이기 위한 조치입니다. 세션 전용 로그인(24시간)은 영향이 없습니다.
- 워크스페이스 모드에서 브라우저(IndexedDB)에 캐시해 둔 개인키/워크스페이스키에 4시간 idle 만료를 추가했습니다. 마지막 사용 후 4시간 동안 아무 동작이 없으면 캐시된 키가 삭제되며, 이후 콘텐츠 접근 시에는 비밀번호를 다시 입력해 키를 재도출해야 합니다(비밀번호 변경 직후와 동일한 흐름). 실제로 사용 중인 세션은 접근할 때마다 만료 시각이 갱신되어 영향받지 않습니다.
