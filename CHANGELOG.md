...
- oeProxy forward proxy: added an admin-configurable relay whitelist (Settings > oeProxy - Forward Proxy). One domain per line, with `*.` wildcard prefix matching the domain and its subdomains; when non-empty, only whitelisted destinations may be relayed and all others get a 403 rendered as a branded oeProxy error page (matching the reverse proxy's existing 404/502/503 pages). Empty (default) keeps prior unrestricted behavior.
